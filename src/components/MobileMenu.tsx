...
export default function MobileMenu() {
  ...
  return (
    <div className="md:hidden">
      ...
      {/* Side Sheet */}
      <div 
        className={`fixed top-0 right-0 z-[99999] h-[100dvh] w-[80%] max-w-[300px] bg-white shadow-[-20px_0_80px_rgba(0,0,0,0.2)] transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
          isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <div className="flex flex-col h-full bg-white relative">
          {/* Top Bar - Solid & Pinned */}
          <div className="flex items-center justify-between px-6 h-20 border-b border-slate-100 bg-white flex-shrink-0">
            ...
            <button
              onClick={() => setIsOpen(false)}
              className="p-3 text-slate-400 hover:text-brand transition-all rounded-2xl bg-slate-50 border border-slate-100"
            >
          ...
          <div className="p-6 border-t border-slate-100 bg-white space-y-3 flex-shrink-0 mb-safe">
            <Link
              href="/register"
            ...
            <Link
              href="/login"
              className="secondary-button w-full h-14 flex items-center justify-center text-sm font-bold border border-slate-200 bg-white"
            >
              Sign In
            </Link>
