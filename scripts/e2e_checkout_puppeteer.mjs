...
async function fillStripeCheckout(page) {
  ...
  try {
    const linkBtn = await page.$x("//a[contains(., 'Pay without Link')] | //button[contains(., 'Pay without Link')]");
    if (linkBtn.length) { await linkBtn[0].click().catch(()=>{}); await sleep(800); }
  } catch {}
  ...
  for (let attempt=0; attempt<8; attempt++) {
    ...
    const ready = await page.$x("//button[not(@disabled) and (contains(., 'Subscribe') or contains(., 'Pay'))]");
  ...
  for (const xp of submitCandidates) {
    ...
    if (el.length) { await el[0].click().catch(()=>{}); break; }
  }
