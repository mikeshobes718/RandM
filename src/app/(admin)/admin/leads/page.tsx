    ...
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="flex flex-col gap-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div>
            <h1 className="text-3xl font-black text-on-surface tracking-tight">Performance Pool</h1>
            <p className="text-on-surface-variant font-medium mt-1">Real-time metrics across all sales representatives.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={stat.label} className="bg-surface p-8 rounded-[32px] border border-outline-variant/20 shadow-xl shadow-outline-variant/20 text-center group">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant/60 mb-2 group-hover:text-brand transition-colors">{stat.label}</p>
      ...
      <div className="bg-surface rounded-[40px] border border-outline-variant/20 shadow-xl shadow-outline-variant/20 overflow-hidden">
        <div className="p-8 border-b border-outline-variant/20">
          <h2 className="text-xs font-black uppercase tracking-[0.2em] text-on-surface flex items-center gap-2">
        ...
        <div className="overflow-x-auto max-h-[400px] overflow-y-auto custom-scrollbar relative">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant/20 text-sm">
              {repMetrics.map((rep) => (
                <tr key={rep.rep_email} className="hover:bg-surface-container-lowest/50 transition-colors group">
                  <td className="px-8 py-5">
                    <p className="font-bold text-on-surface">{rep.rep_name}</p>
                    <p className="text-[10px] text-on-surface-variant/60 font-medium">{rep.rep_email}</p>
                  </td>
              ...
              {repMetrics.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-8 py-12 text-center text-on-surface-variant/60 font-bold italic">
      ...
      <div className="bg-surface rounded-[40px] border border-outline-variant/20 shadow-xl shadow-outline-variant/20 overflow-hidden">
        <div className="p-8 border-b border-outline-variant/20 flex flex-wrap items-center justify-between gap-4">
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-on-surface-variant/60">Leads from Call Log</h3>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant/20 text-sm">
              {filteredLeads.map((lead) => (
                <tr key={lead.id} className="hover:bg-surface-container-lowest/50 transition-colors group">
                  <td className="px-8 py-5">
                    <p className="font-bold text-on-surface">{lead.name}</p>
                    <p className="text-xs text-on-surface-variant/60 font-medium">{lead.city}, {lead.state}</p>
                  </td>
                  <td className="px-4 py-5 text-center font-bold text-on-surface-variant">
                    {lead.times_called || 0}
                  </td>
                  <td className="px-4 py-5">
                    <span className="text-on-surface font-bold">{lead.assigned_to_name || 'Unassigned'}</span>
                  </td>
