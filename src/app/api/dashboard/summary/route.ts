...
export async function GET(req: NextRequest) {
  ...
  let { data: biz, error } = await supa
    .from('businesses')
    .select('id,name,slug,review_link,google_maps_write_review_uri,contact_phone,google_rating,google_place_id')
    .eq('owner_uid', uid)
    .maybeSingle();

  // Fallback: if slug column doesn't exist yet, retry without it
  if (error && /column|slug/.test(error.message || '')) {
    const fallback = await supa
      .from('businesses')
      .select('id,name,review_link,google_maps_write_review_uri,contact_phone,google_rating,google_place_id')
      .eq('owner_uid', uid)
      .maybeSingle();
    biz = fallback.data as any;
  ...
  if (biz && !biz.slug && biz.name) {
    const generatedSlug = (biz.name as string).toLowerCase().replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-').replace(/-+/g, '-').slice(0, 50);
    try {
      await supa.from('businesses').update({ slug: generatedSlug }).eq('id', biz.id);
      (biz as any).slug = generatedSlug;
    } catch {}
  }
