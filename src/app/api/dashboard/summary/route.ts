  ...
  try {
    ...
    try {
      ...
      if (userData?.email?.toLowerCase() === 'bladespindler@gmail.com') {
        ...
        if ((planStatus === 'active' || planStatus === 'trialing') && planId !== 'starter' && planId !== 'free') {
          isPro = true;
        }
      }
    } catch (e) {
    ...
    const bizResult = await supa
      .from('businesses')
      .select('id,name,slug,review_link,google_maps_write_review_uri,google_rating,google_place_id,contact_phone,google_photo_url,address,business_type')
      .eq('owner_uid', uid)
      .maybeSingle();

    if (bizResult.error) {
      if (bizResult.error.message.includes('google_photo_url') || bizResult.error.message.includes('address') || bizResult.error.message.includes('business_type') || bizResult.error.message.includes('slug')) {
