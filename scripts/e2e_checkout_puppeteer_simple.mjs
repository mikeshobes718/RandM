...
async function fillStripeCheckout(page) {
  ...
  try {
    const linkBtn = await page.$('a:has-text("Pay without Link"), button:has-text("Pay without Link")');
    if (linkBtn) { 
  ...
  for (let attempt=0; attempt<8; attempt++) {
    ...
    const ready = await page.$('button:not([disabled]):has-text("Subscribe"), button:not([disabled]):has-text("Pay")');
    if (ready) break;
    await sleep(500);
  }

  // Press Subscribe / Pay
  const submitButton = await page.$('button:has-text("Subscribe"), button:has-text("Pay"), button[type="submit"]');
  if (submitButton) {
