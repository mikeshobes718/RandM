...
const MISCONFIG_MSG =
  'This site is using the Supabase anon key where the service role key is required. In Vercel → Environment Variables, set SUPABASE_SERVICE_ROLE_KEY to the "service_role" secret from Supabase → Project Settings → API (Reveal).';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (peekSupabaseServiceKeyRole() !== 'service_role') {
      console.error('[upsert/form] SUPABASE_SERVICE_ROLE_KEY is not service_role');
      return NextResponse.json({ ok: false, error: MISCONFIG_MSG }, { status: 503 });
    }
    ...
    try {
      if (uid && email) {
        await supabase.from('users').upsert({ uid, email });
      }
    } catch {
      /* non-fatal */
    }
    ...
    let { error }: { error: PostgrestError | null } = await supabase
      .from('businesses')
    ...
    if (error && /ON CONFLICT/.test((error as { message?: string }).message || '')) {
      try {
        ...
        if (existing) {
          ...
          const { error: insertError } = await supabase.from('businesses').insert(payloadRow);
          error = insertError;
        }
      } catch (e: unknown) {
    ...
    if (error) {
      return NextResponse.json(
        { ok: false, error: error.message || 'Could not save business' },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      res.headers.set(
        'Set-Cookie',
        `onboarding_complete=1; Path=/; Max-Age=${60 * 60 * 24 * 365}; SameSite=Lax${domain}`
      );
    } catch {
