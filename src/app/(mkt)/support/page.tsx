    ...
    <main className="relative min-h-screen overflow-hidden bg-gradient-to-br from-white via-indigo-50 to-white">
      ...
      <section className="relative px-4 pt-24 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/80 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.4em] text-slate-600 shadow-sm shadow-slate-900/5 backdrop-blur">Support</span>
          <h1 className="mt-6 text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">We’re here for every review cycle</h1>
          <p className="mt-4 text-lg text-slate-600 md:text-xl">Access live specialists, deep documentation, and guided playbooks whenever you need them.</p>
        </div>
      </section>
      ...
      <section className="px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-5xl gap-8 md:grid-cols-[minmax(0,1.4fr),minmax(0,1fr)]">
          <div className="rounded-3xl border border-white/70 bg-white/85 p-8 shadow-lg shadow-slate-900/10 backdrop-blur">
            ...
            <p className="mt-3 text-sm text-slate-600 leading-relaxed">Prefer email? Reach our specialists at <a href="mailto:support@reviewsandmarketing.com" className="font-semibold text-indigo-600 hover:underline">support@reviewsandmarketing.com</a>. We respond within one business day.</p>
            <div className="mt-6 grid gap-4 rounded-2xl border border-slate-200/80 bg-white/80 p-5 text-sm text-slate-600 shadow-inner">
              <div>
