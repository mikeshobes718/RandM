...
export default function AdminUploadLeads() {
  ...
  return (
    <div className="max-w-4xl animate-fade-in">
      ...
      <div className="mb-10">
        <h1 className="text-3xl font-black text-slate-900 tracking-tight">Upload New Leads</h1>
        <p className="text-slate-500 font-medium mt-1">Bulk import leads from a CSV file into the pool.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl shadow-slate-200/40">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-900 mb-6">1. Get the template</h3>
            <p className="text-sm text-slate-500 font-medium mb-6">Use our CSV template to ensure your leads are imported correctly.</p>
            <button className="w-full py-4 rounded-2xl bg-slate-50 border border-slate-100 text-xs font-black uppercase tracking-widest hover:bg-slate-100 transition-all flex items-center justify-center gap-2">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-xl shadow-slate-200/40">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-900 mb-8">2. Upload your file</h3>
            
            <form onSubmit={handleSubmit} className="space-y-10">
              <label className="group block cursor-pointer">
                <input type="file" accept=".csv" className="hidden" onChange={handleFileChange} />
                <div className="border-4 border-dashed border-slate-50 rounded-[32px] p-12 flex flex-col items-center justify-center group-hover:bg-slate-50 group-hover:border-slate-100 transition-all">
                  <div className="w-16 h-16 rounded-3xl bg-slate-50 flex items-center justify-center text-slate-300 mb-4 group-hover:scale-110 transition-all group-hover:bg-brand/10 group-hover:text-brand">
                  ...
                  <p className="text-lg font-black text-slate-900">{file ? file.name : "Click to select CSV"}</p>
                  <p className="text-sm text-slate-400 font-medium mt-1">or drag and drop your file here</p>
                </div>
              </label>

              {preview.length > 0 && (
                <div className="space-y-4 animate-fade-in">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Preview (First 10 rows)</h4>
