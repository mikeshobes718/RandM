    ...
    <main className="relative min-h-screen overflow-hidden bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      ...
      <section className="relative px-4 pt-24 pb-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/90 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.4em] text-slate-700 shadow-sm shadow-slate-900/5 backdrop-blur">
            Terms
          </span>
      ...
      <section className="px-4 pb-20 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl space-y-6">
          {TERMS.map(({ heading, body }) => (
            <div key={heading} className="rounded-3xl border border-slate-200 bg-white/90 p-8 shadow-lg shadow-slate-900/10 backdrop-blur">
              <h2 className="text-lg font-semibold text-slate-900">{heading}</h2>
              <p className="mt-2 text-sm text-slate-700 leading-relaxed">{body}</p>
            </div>
          ))}
          <div className="rounded-3xl border border-slate-200 bg-white/90 p-8 text-sm text-slate-700 shadow-lg shadow-slate-900/10 backdrop-blur">
